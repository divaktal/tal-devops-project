...
class SimpleAdmin {
    ...
    async loadScheduleData() {
        ...
        try {
            ...
            const response = await fetch(`${this.apiBase}/appointments/timeline?date=${date}`);
            const data = await response.json();
            ...
            if (data.success) {
                ...
                alert('Failed to load schedule: ' + data.error);
                const container = document.getElementById('timelineContainer');
                if (container) {
                    container.innerHTML = `
                        <div style="text-align: center; padding: 40px; color: #666;">
                ...
                html += `
                    <div class="timeline-item">
                        ...
                        <div class="timeline-actions">
                            ${slot.appointment ? 
                              `<button class="btn-delete-sm" onclick="admin.deleteAppointmentFromTimeline(${slot.appointment.id})" style="padding: 4px 8px; font-size: 12px;">Delete</button>` : 
                              ''}
                        </div>
                    </div>
                `;
    ...
    async deleteAppointmentFromTimeline(id) {
        ...
        try {
            ...
            if (response.ok && data.success) {
                alert('Appointment deleted successfully!');
                this.loadScheduleData(); // Refresh the schedule view
